...
int Encoder2_overflow_count_init;	
...
M2_speed_PID_wifiset->Kp=22.0;
M2_speed_PID_wifiset->Ki=2.0;
M2_speed_PID_wifiset->Kd=1.0;
xQueueOverwrite(Motor2_PID_Parameter_Queue,(void *)&M2_speed_PID_wifiset);
	
/*编码器溢出计数初始化*/
Encoder1_overflow_count_init=0;
xQueueOverwrite(Encoder1_Overflow_Queue,&Encoder1_overflow_count_init);

Encoder2_overflow_count_init=0;
xQueueOverwrite(Encoder2_Overflow_Queue,&Encoder2_overflow_count_init);
